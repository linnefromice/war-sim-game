...
| # | 項目 | 工数 | 概要 | 参考 |
|---|------|------|------|------|
| F5 | セーブ/ロード拡張 | 中 | 基本版実装済み（PR #24）。オートセーブ（ターン末自動）+ スロット制（3枠）への拡張 | — |
...
| ~~F14~~ | ~~ユニットバランス調整~~ | ~~小~~ | ~~Hero 4体 vs Villain 9体の不均衡是正~~ — **PR #24 で完了** | — |
...
| # | 項目 | 工数 | 概要 |
|---|------|------|------|
| A11 | キーボード操作の完全対応 | 中 | 基本操作実装済み（PR #24: 矢印キー/Enter/Esc/Tab）。残: 数字キーで武装選択 |
...
| # | 項目 | 概要 |
|---|------|------|
| F14 | ユニットバランス調整 | Hero fighter HP 800→1000。Villain fighterと同等に統一 |
| F5基本版 | セーブ/ロード | LocalStorage永続化。TurnBannerに保存ボタン、DifficultySelectにロードボタン |
| A11基本版 | キーボード操作 | 矢印キーでグリッドカーソル移動、Enter選択、Escキャンセル、Tab+カーソル連動 |
